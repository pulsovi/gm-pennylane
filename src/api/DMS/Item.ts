...
export class APIDMSItem {
  ...
  public static Create(d: any, field?: string, multiple ?: string): APIDMSItem {
    ...
    const knownProperties = ["archived_at","children","comments_count","created_at","creator","favorite","file_extension","file_size","file_url","filters","fixed","id","imports_allowed","itemable_id","method","name","parent_id","pusher_channel","readonly","reference_link","shared","signed_id","sort","type","updated_at","visible"];
...
export class ChildrenEntity {
  ...
  public static Create(d: any, field?: string, multiple ?: string): ChildrenEntity {
    ...
    const knownProperties = ["archived_at","comments_count","created_at","creator","favorite","file_extension","file_size","file_url","files_count","fixed","id","imports_allowed","itemable_id","method","name","parent_id","pusher_channel","readonly","reference_link","shared","signed_id","type","updated_at","visible"];
    const unknownProperty = Object.keys(d).find(key => !knownProperties.includes(key));
    if (unknownProperty) errorHelper(field + '.' + unknownProperty, d[unknownProperty], "never (unknown property)");
