...
import { getReact, getReactProps, getReactPropValue } from "../../_/react.js";
...
export default class HighlightWrongDMSFilenames extends Service {
  ...
  async watch() {
    while (await waitPage("transactionDetail")) {
      const unmanagedDMSItems = $$(".ui-card:not(.GM) span.tiny-caption").filter(
        (span) => !span.classList.contains("GM") && span.innerText.startsWith("ajouté dans la GED le ")
      );
      ...
      for (const span of unmanagedDMSItems) {
        ...
        const files = getReactPropValue(span, "files") as APIDMSLink[];
